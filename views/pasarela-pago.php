...
$detalles = [];
$mensaje_error = '';
require_once '../php/pagos/pasarela.php';
      ...
      <?php else: ?>
        ...
        <form action="../php/pagos/procesar_pago.php" method="POST" id="form-pasarela-pago">
          ...
          <p>
            ...
            <input type="email" id="email" name="email" value="<?= htmlspecialchars($currentUser['email'] ?? '') ?>" placeholder="correo@ejemplo.com" required />
          </p>
          <p>
            <label for="numero_tarjeta"><strong>Número de tarjeta:*</strong></label><br />
            <input type="text" id="numero_tarjeta" name="numero_tarjeta" placeholder="1234 5678 9012 3456" pattern="[0-9\s]{13,19}" maxlength="19" inputmode="numeric" autocomplete="cc-number" required />
          </p>
          <div class="flex-row">
            <div>
              <label for="fecha_expiracion"><strong>Expiración:*</strong></label><br />
              <input type="text" id="fecha_expiracion" name="fecha_expiracion" placeholder="MM/AA" pattern="(0[1-9]|1[0-2])\/[0-9]{2}" maxlength="5" inputmode="numeric" autocomplete="cc-exp" required />
            </div>
            <div>
              <label for="cvv"><strong>CVV:*</strong></label><br />
              <input type="password" id="cvv" name="cvv" placeholder="123" pattern="[0-9]{3,4}" maxlength="4" inputmode="numeric" autocomplete="cc-csc" required />
            </div>
          </div>
          <button type="submit" class="btn btn-full mt-section">Pagar $<?= number_format($monto_total, 2, ',', '.') ?></button>
        </form>
      <?php endif; ?>

      <hr />
      <p class="text-center"><a href="carrito.php" class="link">← Volver al Carrito</a></p>
